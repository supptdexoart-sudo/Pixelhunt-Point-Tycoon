...
const CORRUPTED_COLORS = ['#f3e8ff', '#c084fc', '#a855f7', '#9333ea', '#7e22ce', '#581c87', '#4c1d95', '#3b0764', '#2e1065'];
...
export const DynamicDiamond: React.FC<DynamicDiamondProps> = ({ level, hpPercentage, isShaking, isExploding, isHit, isCorrupted }) => {
    ...
    const colors = isCorrupted ? CORRUPTED_COLORS : config.colors;
    const glowColor = isCorrupted ? CORRUPTED_COLORS[3] : config.colors[4]; // Pick a vibrant color from the palette for the main glow
    ...
    return (
        ...
        >
            <div
                className={`gem-wrapper ${isCorrupted ? 'corrupted' : ''}`}
                style={{ '--glow-color': glowColor } as React.CSSProperties}
            >
                <div className="absolute inset-0">
                    <DiamondSVG colors={colors} />
                </div>
                <div
