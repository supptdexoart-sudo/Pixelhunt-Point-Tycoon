...
import { FloatingReward as FloatingRewardType } from '../types';
import { useTranslation } from '../contexts/LanguageContext';
import { ShardIcon } from './icons/ShardIcon';

interface FloatingRewardProps {
  reward: FloatingRewardType;
  onClick: (reward: FloatingRewardType, x: number, y: number) => void;
}

const SmallDiamondIcon: React.FC = () => {
...
export const FloatingReward: React.FC<FloatingRewardProps> = ({ reward, onClick }) => {
  const { t } = useTranslation();

  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    onClick(reward, e.clientX, e.clientY);
  };
  
  let content: React.ReactNode;
  let buttonClass = "floating-reward ";

  switch (reward.type) {
      case 'shard':
          content = <div className="w-8 h-8"><ShardIcon /></div>;
          buttonClass += "w-10 h-10"; // smaller for shard
          break;
      case 'jackpot':
          content = <SmallDiamondIcon />;
          buttonClass += "w-14 h-14 jackpot-glow"; // bigger and glowing for jackpot
          break;
      default: // 'points'
          content = <SmallDiamondIcon />;
  ...
  return (
    <button
      ...
      onClick={handleClick}
      aria-label={t('floating_reward_aria')}
    >
        {content}
    </button>
  );
